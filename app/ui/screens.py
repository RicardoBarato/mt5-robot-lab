...
SCREEN_TITLES = {
    ...
    "mt5_setup": "MT5 Setup",
    "real_mt5_smoke_gate": "Real MT5 Smoke Gate",
...
def build_screen_registry(config: AppConfig) -> list[ScreenDefinition]:
    ...
    return [
        ...
        ScreenDefinition(
            id="real_mt5_smoke_gate",
            title="Real MT5 Smoke Gate",
            subtitle="Preview explicit approval requirements before any future real MT5 smoke.",
            cards=(
                ("MT5 detected", "preview only"),
                ("Terminal found", "preview only"),
                ("MetaEditor found", "preview only"),
                ("Operator approval", "required"),
                ("Smoke only", "true"),
                ("Max backtests", "1"),
            ),
            body_lines=(
                "Buttons: Preview Real MT5 Smoke Requirements, Generate Approval Manifest, Return to Safe Mode.",
                "This MVP does not add a button that runs MT5 real execution.",
            ),
            primary_action="Preview Real MT5 Smoke Requirements",
        ),
        ScreenDefinition(
            id="symbol_timeframe",
